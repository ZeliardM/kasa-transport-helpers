...
def make_fake_cert(public_bytes_value: bytes, aia_value: object) -> object:
    class FakeExts:
...
def test_fetch_root_ca_aia_non_ca_issuers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        noc.crypto_x509, "load_pem_x509_certificate", lambda b: fake_cert
    )
    res = noc._fetch_root_ca("host")
    assert res == b"PEM1"


def test_fetch_root_ca_aia_access_location_not_uri(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ad = types.SimpleNamespace(
        access_method=noc.AuthorityInformationAccessOID.CA_ISSUERS,
...
def test_fetch_root_ca_url_fetch_parse_fail(monkeypatch: pytest.MonkeyPatch) -> None:
    class URI:
